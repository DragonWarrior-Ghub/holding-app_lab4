...
3. Запустите dev-среду командой `npm start` — она параллельно поднимет API на `:4000` (порт меняется через `HOLDING_API_PORT`) и React-приложение на `:3000`.
4. Если нужен только сервер без фронтенда, используйте `npm run server`.
...
> `npm start` останавливает и фронтенд, и API, если один из процессов падает (например, из-за отсутствующего файла БД), поэтому не будет зависших прокси-ошибок.
